...
async function generateTextImage(args = {}) {
	...
	if (args.color) {
		if (!isValidHexColor(args.color)) {
			throw new Error(`Invalid hex color format: ${args.color}. Must be in format #RRGGBB or #RGB`);
		}
...
export default async function handler(req, res) {
	...
	// Method not allowed
	return res.status(405).json({ error: 'Method not allowed. Use GET for capabilities or POST for generation.' });
}
